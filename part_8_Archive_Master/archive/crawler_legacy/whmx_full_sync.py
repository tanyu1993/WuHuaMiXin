...
def sync_character(name):
    ...
    try:
        # 使用 curl_cffi 模拟真实请求
        r = requests.get(reader_url, impersonate="chrome", timeout=30)
...
if __name__ == "__main__":
    ...
    for char in character_list:
        if sync_character(char):
            count += 1
        time.sleep(2)  # 礼貌性延迟
    print(f"\nFinished! Successfully synced {count} characters.")
