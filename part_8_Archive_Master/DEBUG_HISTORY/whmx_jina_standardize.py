...
def sync_jina(name):
    # 强制统一使用 Jina Reader
    reader_url = f"https://r.jina.ai/https://wiki.biligame.com/whmx/{name}"
    print(f"Jina Standardization: {name} ...", flush=True)
    ...
    for attempt in range(3):
        try:
            ...
            if r.status_code == 200:
                ...
                print(f"  Attempt {attempt+1} failed: {r.status_code}", flush=True)
...
if __name__ == "__main__":
    for char in character_list:
        sync_jina(char)
        time.sleep(15) # Jina 有更严的速率限制，调慢速度以保稳定
