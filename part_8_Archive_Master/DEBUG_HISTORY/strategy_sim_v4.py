...
class WHMXFinalSim:
    ...
    def run_simulation(self, years=1000):
        ...
        for year in range(1, years + 1):
            ...
            for i, p_type in enumerate(pools_config):
                ...
                # 收入结算
                income_pulls = self.INCOME_PULLS_PER_POOL
                income_reds = self.INCOME_REDCARDS_PER_POOL
                self.inventory_pulls += income_pulls
                self.inventory_red_cards += income_reds
                ...
                while True:
                    ...
                    if p_type == "limited":
                        if up_copies >= target:
                            if pulls_spent >= 160: up_copies += (pulls_spent // 160)
                            else: self.inventory_points += (pulls_spent * 10)
                            break
                ...
                self.history.append({
                    ...
                    "初始红卡": round(init_reds, 1),
                    "期末红卡": round(self.inventory_red_cards, 1),
                    "初始积分": int(init_points),
    ...
    def save_to_excel(self, filename):
        ...
        # 颜色定义
        red_fill = PatternFill(start_color="FFCCCC", end_color="FFCCCC", fill_type="solid") # 歪了
        green_fill = PatternFill(start_color="CCFFCC", end_color="CCFFCC", fill_type="solid") # 成功
...
if __name__ == "__main__":
    ...
    sim.save_to_excel("whmx/物华弥新_1000年人生模拟报告.xlsx")
    print("报告已生成！")
