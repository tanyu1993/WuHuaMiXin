...
class WHMXFinalSimV9:
    def __init__(self):
        ...
        # 待补强清单: [{"pool_idx": idx, "type": type, "copies": n}]
        ...
        for year in range(1, years + 1):
            ...
            for i, p_type in enumerate(pools_config):
                ...
                for item in self.pending_upgrades:
                    while item["copies"] < 4:
                        if self.inventory_red_cards >= 4:
                            self.inventory_red_cards -= 4; item["copies"] += 1; item["supp_history"] += 1
                            reds_spent_month += 4; supp_copies_month += 1
                        elif self.inventory_points >= 4000:
                            self.inventory_points -= 4000; item["copies"] += 1; item["supp_history"] += 1
                            points_spent_month += 4000; supp_copies_month += 1
                        else: break
                ...
                self.history.append({
                    ...
                    "当期UP数": up_copies,
                    "红卡补强": supp_copies_month,
                    "歪了没": "是" if spooks > 0 else "否",
                    "出货流水": " | ".join(pity_records),
                    "消耗红卡": reds_spent_month,
                    "消耗积分": points_spent_month,
    ...
    def save(self, filename):
        ...
        for row in ws.iter_rows(min_row=2, max_row=ws.max_row):
            if row[10].value == "是": row[10].fill = red_fill
        wb.save(filename)

if __name__ == "__main__":
    sim = WHMXFinalSimV9()
    sim.run_simulation(1000)
    sim.save("whmx/物华弥新_1000年人生模拟报告_B档最终版.xlsx")
    print("报告已生成！")
