...
def debug_image(img_path):
    ...
    print(f"Detected U: {U:.1f}, Rows: {rows}")
    ...
    for gp in grid_points:
        ...
        cv2.putText(canvas, f"{gp['row']},{gp['col']}", (x1, y1+30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0,0,255), 2)

    save_path = os.path.join(PROJECT_ROOT, 'whmx', 'vision', 'debug_tamako.jpg')
    cv2.imencode('.jpg', canvas)[1].tofile(save_path)
    print(f"Debug saved to: {save_path}")

if __name__ == "__main__":
    img = os.path.join(PROJECT_ROOT, 'whmx', 'accounts', 'Tamako', '微信图片_20260228152826.jpg')
    debug_image(img)
