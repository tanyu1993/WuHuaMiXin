...
def comprehensive_test():
    ...
    BASE_DIR = os.getcwd()
    ACCOUNTS_ROOT = os.path.join(BASE_DIR, 'whmx', 'accounts')
    
    if not os.path.exists(ACCOUNTS_ROOT):
        print(f"[!] 找不到账号目录: {ACCOUNTS_ROOT}")
        return
    ...
    # 2. 遍历所有账号文件夹
    accounts = [d for d in os.listdir(ACCOUNTS_ROOT) if os.path.isdir(os.path.join(ACCOUNTS_ROOT, d))]
    print(f"[*] 发现 {len(accounts)} 个账号，准备进行抽样测试...\n")

    for acc_name in accounts:
        acc_path = os.path.join(ACCOUNTS_ROOT, acc_name)
        # 获取支持的图片格式
